...
from resqs.s3 import s3_backends
from resqs.s3.models import get_moto_s3_account_id
...
class S3ConfigQuery(ConfigQueryModel):
    def list_config_service_resources(
        ...
        if not next_token:
            ...
            # Tokens for this resqs feature is just the bucket name:
...
class S3AccountPublicAccessBlockConfigQuery(ConfigQueryModel):
    def list_config_service_resources(
        ...
        if not next_token:
            ...
            # Tokens for this resqs feature is just the region-name:
            # For OTHER non-global resource types, it's the region concatenated with the resource ID.
