...
from resqs.core.exceptions import DryRunClientError
...
from resqs.core.utils import camelcase_to_underscores, method_names_from_class
...
class ActionAuthenticatorMixin(object):
    ...
    def _authenticate_and_authorize_normal_action(self):
        from resqs.iam.access_control import IAMRequest

        self._authenticate_and_authorize_action(IAMRequest)

    def _authenticate_and_authorize_s3_action(self):
        from resqs.iam.access_control import S3IAMRequest
            ...
            def wrapper(*args, **kwargs):
                if settings.TEST_SERVER_MODE:
                    response = requests.post(
                        "http://localhost:5000/resqs-api/reset-auth",
...
class MotoAPIResponse(BaseResponse):
    def reset_response(self, request, full_url, headers):
        ...
        return 400, {}, json.dumps({"Error": "Need to POST to reset resqs"})
    ...
    def reset_auth_response(self, request, full_url, headers):
        ...
        return 400, {}, json.dumps({"Error": "Need to POST to reset resqs Auth"})

    def model_data(self, request, full_url, headers):
        from resqs.core.models import model_data
