...
from resqs.dynamodb2.parsing.reserved_keywords import ReservedKeywords
...
class UpdateExpressionFunctionEvaluator(DepthFirstTraverser):
    ...
    def process_function(self, node):
        ...
        if function_name == "if_not_exists":
            ...
            raise NotImplementedError(
                "Unsupported function for resqs {name}".format(name=function_name)
            )
    ...
    def process_update_expression_value(self, node):
        ...
        if len(node.children) == 1:
            ...
            if operator == "+":
                ...
                raise NotImplementedError(
                    "resqs does not support operator {operator}".format(
                        operator=operator
                    )
                )
        else:
