...
from resqs.dynamodb2.models import DynamoType
from resqs.dynamodb2.models.dynamo_type import DDBTypeConversion, DDBType
from resqs.dynamodb2.parsing.ast_nodes import (
...
from resqs.dynamodb2.parsing.validators import ExpressionPathResolver
    ...
    def set(
        ...
        if isinstance(element_to_set, ExpressionAttribute):
            ...
            raise NotImplementedError(
                "resqs does not support setting {t} yet".format(t=type(element_to_set))
            )
    ...
    def execute(self, item):
        ...
        if not string_set_list:
            ...
            if isinstance(element, ExpressionAttributeName):
                ...
                raise NotImplementedError(
                    "resqs does not support deleting {t} yet".format(t=type(element))
                )
...
class RemoveExecutor(NodeExecutor):
    def execute(self, item):
        ...
        if isinstance(element_to_remove, ExpressionAttribute):
            ...
            raise NotImplementedError(
                "resqs does not support setting {t} yet".format(
...
class UpdateExpressionExecutor(object):
    ...
    def execute(self, node=None):
        """
        As explained in resqs.dynamodb2.parsing.expressions.NestableExpressionParserMixin._create_node the order of nodes
