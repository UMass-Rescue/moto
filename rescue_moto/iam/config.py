import json
import boto3
from rescue_moto.core.exceptions import InvalidNextTokenException
from rescue_moto.core.models import ConfigQueryModel
from rescue_moto.iam import iam_backends
...
class RoleConfigQuery(ConfigQueryModel):
    def list_config_service_resources(
        ...
        # Stored in rescue_moto backend with the AWS-assigned random string like "AROA0BSVNSZKXVHS00SBJ"
        ...
        if aggregator:
            ...
            # we can't really tell if this has happened in rescue_moto, we'll just bind this to the regions in your aggregator
            aggregated_regions = []
...
class PolicyConfigQuery(ConfigQueryModel):
    def list_config_service_resources(
        ...
        # Stored in rescue_moto backend with the arn like "arn:aws:iam::123456789012:policy/my-development-policy"
        ...
        # but it's default set be default, and rescue_moto's config doesn't yet support
