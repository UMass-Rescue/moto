...
from rescue_moto.dynamodb2.models import DynamoType
from rescue_moto.dynamodb2.models.dynamo_type import DDBTypeConversion, DDBType
from rescue_moto.dynamodb2.parsing.ast_nodes import (
...
from rescue_moto.dynamodb2.parsing.validators import ExpressionPathResolver
    ...
    def set(
        ...
        if isinstance(element_to_set, ExpressionAttribute):
            ...
            raise NotImplementedError(
                "rescue_moto does not support setting {t} yet".format(t=type(element_to_set))
            )
    ...
    def execute(self, item):
        ...
        if not string_set_list:
            ...
            if isinstance(element, ExpressionAttributeName):
                ...
                raise NotImplementedError(
                    "rescue_moto does not support deleting {t} yet".format(t=type(element))
                )
...
class RemoveExecutor(NodeExecutor):
    def execute(self, item):
        ...
        if isinstance(element_to_remove, ExpressionAttribute):
            ...
            raise NotImplementedError(
                "rescue_moto does not support setting {t} yet".format(
...
class UpdateExpressionExecutor(object):
    ...
    def execute(self, node=None):
        """
        As explained in rescue_moto.dynamodb2.parsing.expressions.NestableExpressionParserMixin._create_node the order of nodes
