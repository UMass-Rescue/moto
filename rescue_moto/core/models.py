...
from rescue_moto import settings
import responses
from rescue_moto.packages.httpretty import HTTPretty
from rescue_moto.compat import patch
from .utils import (
...
class BaseMockAWS(object):
    ...
    def __init__(self, backends):
        from rescue_moto.instance_metadata import instance_metadata_backend
        from rescue_moto.core import moto_api_backend
...
# Default behaviour is to return subsequent matches for subsequent requests, which leads to https://github.com/spulec/rescue_moto/issues/2567
...
class ServerModeMockAWS(BaseMockAWS):
    def reset(self):
        ...
        requests.post("http://localhost:5000/rescue_moto-api/reset")
...
class ConfigQueryModel(object):
    ...
    def list_config_service_resources(
        ...
        This only lists resources within a region. The way that this is implemented in rescue_moto is based on the region
        ...
        aggregator can aggregate resources from multiple regions. In rescue_moto, aggregated regions will *assume full aggregation
        ...
        :param aggregator: If the query is an aggregated query, *AND* the resource has "non-standard" aggregation logic (mainly, IAM),
                                ...
                                conditional logic under `if aggregator` in the rescue_moto/iam/config.py for the IAM example.
    ...
    def get_config_resource(
        ...
        This only fetches a resource config within a region. The way that this is implemented in rescue_moto is based on the region
        for the resource backend.
...
class MotoAPIBackend(BaseBackend):
    def reset(self):
        import rescue_moto.backends as backends
